...
ggplot(rec_use, aes(x = capture_rate, y = mgp_rate)) +
  geom_point() +
  geom_smooth()

ggplot(rec_use, aes(x = capture_rate, y = paper_rate)) +
  geom_point() +
  geom_smooth()
...
#NOTE - SPLIT INTO TWO (~90% OF DATA IS IN THE LOWER) - EXCLUDING 26 HIGH OUTLIERS 
...
ggplot(df_ghg, aes(x = avg_ghg, y = place)) +
  geom_point()

ggplot(filter(df_build_energy, ghg <= 2952 & borough == "Queens")) +
  geom_histogram(aes(x = ghg)) +
  xlab("Metric Tons of Green House Gas Emissions") +
  ylab("Buildings")


ggplot(filter(df_build_energy, ghg %in% 0:2952 & building_area < 1000000 & `DOF.Gross.Floor.Area..ftÂ..` < 1000000), aes(x = `DOF.Gross.Floor.Area..ftÂ..`, y = ghg)) +
  geom_point() 
