...
ggplot(clean_rebate_data, 
  ...
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 90))
```
semi linear so lets do linear regression to calculate rate. we see seasonality within a year but it's generally trending up.
...
# TODO for interactive piece: take a # to cap off cum. rebate at
cap = 60000000
```
...
ggplot(final_reg,
  ...
  geom_bar(stat = "identity") +
  geom_hline(yintercept = cap, color = "red", linetype="dotted") +
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 90)) +
  xlab("year_quarter") 
```

Will run out between 2021 Q4 and 2022 Q4
```{r}
 write.csv(x=final_reg, file=file.path(getwd(), "regression.csv"))
```
