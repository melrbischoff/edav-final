...
Further, the key dataset used for NYC building energy usage was missing important data. Specifically, not all observations had the borough field populated, and it had nothing on 2D square footage. As such, the first step in our data transformation was merging the dataset with the zip-boroughs mapping dataset and the land-use dataset in order to access the desired information.

In addition, the greenhouse gas (ghg) field had to be converted to a numeric for manipulation and interpretation. After this, we reshaped the data further by grouping by borough and zip code and taking both the mean and median GHG emissions for each. As previously noted, a single negative value for GHG emissions was found. It is filtered out here.
