...
To analyze when the budget will rum out if EV purchases continue at this rate, we perform a linear regression on the model. The data is complete up to 2021 Q1, so we consider quarters after that to be in the future.
```{r echo = F, message = F, warning=F}
...
ggplot(plot_reg, 
  ...
  geom_text(data=subset(clean_rebate_data_by_quarter,
            ...
            size=4.5,
            hjust = -.3) +
  geom_smooth(data=filter(plot_reg, year_quarter > "2017 Q2"),
              aes(x = factor(year_quarter),
                  y = cumulative_rebate_amount,
                  colour="purple",
                  group=1),
              method = "lm", se= FALSE, color = "darkgreen", size = .5) +
  geom_hline(mapping=aes(yintercept=60000000, colour="Current Budget"), linetype="dashed") +
  geom_point(aes(x="2021 Q4", y=60000000),
             colour="red",
             shape=18,
             size=3) +  
